...
export type SkypeIndicatorProps = {
  color?: string;
  size?: number;
  minScale?: number;
  maxScale?: number;
...
export const ActivityIndicator: React.FC<SkypeIndicatorProps> = ({
  color = '#C2C7CC',
  size = 40,
  minScale = 0.2,
  maxScale = 1.0,
  ...
  const renderComponent = useCallback(({ index, count, progress }: {
    index: number;
    count: number;
    ...
    const inputRange = Array.from({ length: frames }, (_, i) => i / (frames - 1));
...
const Indicator: React.FC<IndicatorProps> = ({
  ...
  return (
    <Animated.View style={[style, hidesWhenStopped && { opacity: hideAnimation }]}>
